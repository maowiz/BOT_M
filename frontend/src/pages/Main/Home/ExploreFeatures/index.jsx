...
export default function ExploreFeatures() {
  ...
  const getEmbedCode = () => navigate(paths.settings.embedChatWidgets());

  const openAddContent = async () => {
    // Get first workspace or prompt to create one
    const workspaces = await Workspace.all();
    if (workspaces.length > 0) {
      const firstWorkspace = workspaces[0];
      setSelectedWorkspace(firstWorkspace);
      setOpenTab("documents"); // Open with Documents tab
    } else {
      showToast("Please create a workspace first!", "warning", { clear: true });
      showNewWsModal();
    }
  };
    ...
    <div>
      ...
      <div className="w-full grid grid-cols-1 sm:grid-cols-2 gap-4">
        <FeatureCard
          ...
          description="Generate a script tag to embed your chatbot on any website. Perfect for SaaS deployment."
          primaryAction="Create Embed Widget"
          secondaryAction={null}
          onPrimaryAction={getEmbedCode}
          onSecondaryAction={() => { }}
        />
        <FeatureCard
          title="Train Your Bot"
          description="Upload website links or documents to train your AI assistant with your content."
          primaryAction="Add Content"
          secondaryAction={null}
          onPrimaryAction={openAddContent}
          onSecondaryAction={() => { }}
        />
      </div>
