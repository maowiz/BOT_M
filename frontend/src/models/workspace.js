...
const Workspace = {
  ...
  uploadLinkAdvanced: async function (slug, link, depth = 1, maxLinks = 20) {
    const response = await fetch(`${API_BASE}/ext/website-depth`, {
      method: "POST",
      body: JSON.stringify({ url: link, depth, maxLinks }),
      headers: baseHeaders(),
    });
